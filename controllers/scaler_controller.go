	...
	for _, deploy := range scaler.Spec.Deployments {
		...
		if deployment.Spec.Replicas != &replicas {
			...
			for i := range deployment.Spec.Template.Spec.Containers {
				...
				//添加新的资源
				cpuQuantity, err := resource.ParseQuantity(scaler.Spec.Resources.CPU)
				if err != nil {
					return fmt.Errorf("parse cpu quantity error: %v", err)
				}

				memoryQuantity, err := resource.ParseQuantity(scaler.Spec.Resources.Memory)
				if err != nil {
					return fmt.Errorf("parse memory quantity error: %v", err)
				}
				...
				if !reflect.DeepEqual(container.Resources, newResource) {
					container.Resources = newResource
					needUpdate = true
				}
			}

			if needUpdate || *deployment.Spec.Replicas != replicas {
				err := r.Update(ctx, deployment)
				if err != nil {
					scaler.Status.Status = apiv1alpha1.FAILED
					r.Status().Update(ctx, scaler)
...
func restoreDeployment(scaler *apiv1alpha1.Scaler, r *ScalerReconciler, ctx context.Context) error {
	...
	for _, deploy := range scaler.Spec.Deployments {
		...
		for i := range deployment.Spec.Template.Spec.Containers {
			...
			//解析原始资源
			cpuQuantity, err := resource.ParseQuantity(originalInfo.Resources.CPU)
			if err != nil {
				return fmt.Errorf("parse cpu quantity error: %v", err)
			}
			memoryQuantity, err := resource.ParseQuantity(originalInfo.Resources.Memory)
			if err != nil {
				return fmt.Errorf("parse memory quantity error: %v", err)
			}

			originalResource := corev1.ResourceRequirements{
				Limits: corev1.ResourceList{
					corev1.ResourceCPU:    cpuQuantity,
					corev1.ResourceMemory: memoryQuantity,
				},
			...
			if !reflect.DeepEqual(container.Resources, originalInfo.Resources) {
				container.Resources = originalResource
				needUpdate = true
			}
		}
		if needUpdate {
...
func addAnnotations(scaler *apiv1alpha1.Scaler, r *ScalerReconciler, ctx context.Context) error {
	...
	for _, deploy := range scaler.Spec.Deployments {
		...
		if len(deployment.Spec.Template.Spec.Containers) > 0 {
			...
			originalInfo.Resources = apiv1alpha1.ContainerResources{
